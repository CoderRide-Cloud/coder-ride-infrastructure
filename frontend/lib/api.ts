...
export const memberApi = {
  ...
  getProjects: (memberId: number) =>
    apiClient.get(`/projects/user/${memberId}`),
...
export const adminApi = {
  // Member management
  getPendingMembers: () => apiClient.get("/users/pending"),
  getAllUsers: (params?: any) => apiClient.get("/users", { params }),
  setUserLeadStatus: (userId: number, isLead: boolean) =>
    apiClient.put(`/users/${userId}/lead`, { isLead }),

  getLeads: () => apiClient.get("/users/leads"),
  ...
  // Project management
  getPendingProjects: () => apiClient.get("/projects/pending"),
  approveProject: (
    projectId: number,
    data: { isApproved: boolean; reason?: string }
  ) => apiClient.put(`/projects/${projectId}/approval`, data),
  deleteProject: (projectId: number) =>
    apiClient.delete(`/projects/${projectId}`),

  // Event management
  setEventFeatured: (eventId: number, data: any) =>
    apiClient.put(`/events/${eventId}/featured`, data),

  // Custom Roles management
  getCustomRoles: () => apiClient.get("/roles"),
  getCustomRole: (id: number) => apiClient.get(`/roles/${id}`),
  createCustomRole: (data: any) => apiClient.post("/roles", data),
  ...
  getRoleUsers: (roleId: number) =>
    apiClient.get(`/users/role/${roleId}`),
  updateRolePosition: (roleId: number, data: { newPosition: number }) =>
    apiClient.put(`/roles/${roleId}/position`, data),
};
