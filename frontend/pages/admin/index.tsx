...
export default function AdminDashboard() {
      ...
      switch (activeTab) {
        case "projects":
          if (hasPermission(Permission.MANAGE_PROJECTS)) {
            ...
            const mapMembers = (projects: Project[]) => 
              projects.map(p => {
                const user = usersRes.data.find((u: User) => u.id === p.memberId);
                return {
                  ...p,
                  member: { user, fullName: user?.username } as any
                };
              });
              
            setPendingProjects(mapMembers(pendingProjectsRes.data));
            setAllProjects(mapMembers(allProjectsRes.data));
          }
          break;
        case "events":
  ...
  if (status === "loading" || (loading && !allUsers.length)) {
    return (
      <Layout>
