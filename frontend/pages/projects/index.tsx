...
import { projectApi, tagApi, memberApi } from "@/lib/api";
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [projectsRes, tagsRes, membersRes] = await Promise.all([
        ...
        const mappedProjects = projectsRes.data.map((p: Project) => ({
          ...p,
          member: membersRes.data.find((m: any) => m.userId === p.memberId)
        }));
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    const matchesTags =
      ...
      selectedTags.every((tagId) =>
        project.tags.some((t) => t.id === tagId)
      );
